...
def lemmatize_with_spacy(text: str, lang: str = "es") -> List[str]:
    ...
    # 2. Procesa el texto
    # 3. Extrae el lema de cada token: [token.lemma_ for token in doc]
    return []
...
def compare_stem_vs_lemma(word: str, language: str = "spanish") -> dict:
    ...
    return {
        "original": word,
        "stem": "",  # Usa stem_word_snowball
        "lemma": ""  # Usa lemmatize_with_spacy
    }
...
def normalize_text(text: str, method: str = "lemma", language: str = "spanish") -> str:
    ...
    # Usa stem_sentence() si method == "stem"
    # Usa lemmatize_with_spacy() si method == "lemma"
    return ""
